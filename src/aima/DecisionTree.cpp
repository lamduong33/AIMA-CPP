...
template <class T> DecisionTree<T>::DecisionTree() { nodes = nullptr; }

template <class T>
DecisionTree<T>::DecisionTree(std::string& t_attributeName)
    : nodes{std::map<std::string, DecisionTree>()}, attributeName{std::string()}
{
    this->attributeName = t_attributeName;
    *nodes =
        std::map<std::string, DecisionTree>(); // New map object in stack memory
}

/*Add a leaf to the Decision Tree. Each leaf is a terminal node.
This is where the decision of attributes take place.*/
template <class T>
void DecisionTree<T>::addLeaf(std::string t_attributeValue,
                              std::string t_decision)
{
    // DecisionTreeLeaf newLeaf(t_decision);
    DecisionTree newLeaf(t_decision);
    nodes->insert(
        std::pair<std::string, DecisionTree>(t_attributeValue, newLeaf));
}
...
template <class T>
void DecisionTree<T>::addNode(std::string t_attributeValue, DecisionTree t_tree)
{
    nodes->insert(
        std::pair<std::string, DecisionTree>(t_attributeValue, t_tree));
}

/*====================DECISION TREE LEAF DEFINITIONS=======================*/

template <class T>
DecisionTreeLeaf<T>::DecisionTreeLeaf(std::string t_attributeValue)
{
...
template <class T>
void DecisionTreeLeaf<T>::setAttributeValue(std::string t_attributeValue)
{
    m_attributeValue = t_attributeValue;
}
